    ...
    <main className="min-h-dvh bg-bg pb-20">
      <section className="relative min-h-[70dvh] overflow-hidden sm:min-h-screen">
        ...
        <div className="relative z-10 mx-auto flex min-h-[70dvh] max-w-6xl flex-col justify-end px-5 pb-12 pt-24 sm:min-h-screen sm:px-8 sm:pb-16">
          <p className="text-[11px] tracking-[0.42em] text-gold uppercase">Archive</p>
          <h1 className="display mt-4 max-w-5xl text-5xl leading-[1.02] text-fg text-balance sm:text-7xl lg:text-8xl">
            The myth library
          </h1>
          <p className="font-garamond mt-6 max-w-3xl text-xl text-fg/90 sm:text-2xl">
            Every story in the engine, readable outside the walk. Not a catalog of
            types. A set of mirrors.
          </p>
          <p className="display mt-8 text-4xl text-gold sm:text-5xl lg:text-6xl">
            Forty-two, on purpose.
          </p>
          <p className="mt-4 text-base text-muted sm:text-lg">Psyche is the hinge.</p>
        </div>
      </section>
...
function MythDoor({ myth, featured = false }: { myth: Myth; featured?: boolean }) {
  return (
    <Link
      to="/library/$mythId"
      ...
      className={cn(
        ...
        featured ? "min-h-[62vh] sm:min-h-[70vh]" : "min-h-[52vh] sm:min-h-[58vh]",
      )}
    >
      <img
        src={mythArt(myth.id)}
        alt=""
        decoding="async"
        className="absolute inset-0 h-full w-full bg-bg object-cover transition-transform duration-700 ease-out will-change-transform group-hover:scale-[1.03]"
      />
      ...
      <span className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
        ...
        {featured ? (
          <span className="font-garamond mt-4 block max-w-xl text-lg text-fg/90">
